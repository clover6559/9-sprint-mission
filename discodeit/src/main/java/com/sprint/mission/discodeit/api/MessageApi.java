...
public interface MessageApi {
    ...
    @ApiResponses(
            value = {
                ...
                @ApiResponse(
                        ...
                        description = "Message가 이미 존재함",
                        content =
                                @Content(
                                        examples =
                                                @ExampleObject(value = "Message already exists"))),
            })
    ...
    @ApiResponses(
            value = {
                @ApiResponse(
                        ...
                        description = "Message 정보가 성공적으로 수정됨",
                        content = @Content(schema = @Schema(implementation = Message.class))),
                @ApiResponse(
    ...
    @ApiResponses(
            value = {
                @ApiResponse(responseCode = NO_CONTENT_204, description = "Message가 성공적으로 삭제됨"),
                @ApiResponse(
                        responseCode = NOT_FOUND_404,
                        description = "Message를 찾을 수 없음",
                        content =
                                @Content(
                                        examples =
                                                @ExampleObject(
                                                        value = "Message with id {id} not found")))
            })
