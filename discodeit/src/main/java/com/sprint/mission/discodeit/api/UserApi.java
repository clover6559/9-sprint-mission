...
public interface UserApi {
  ...
  @ApiResponses(value = {
      @ApiResponse(
          responseCode = CREATED_201, description = "User가 성공적으로 생성됨",
          content = @Content(schema = @Schema(implementation = User.class))
      ),
      @ApiResponse(
          responseCode = BAD_REQUEST_400, description = "같은 email 또는 username를 사용하는 User가 이미 존재함",
  ...
  @ApiResponses(value = {
      @ApiResponse(
          responseCode = SUCCESS_200, description = "User 정보가 성공적으로 수정됨",
          content = @Content(schema = @Schema(implementation = User.class))
      ),
      @ApiResponse(
          responseCode = NOT_FOUND_404, description = "User를 찾을 수 없음",
  ...
  @ApiResponses(value = {
      @ApiResponse(
          responseCode = NO_CONTENT_204, description = "User가 성공적으로 삭제됨"
      ),
      @ApiResponse(
          responseCode = NOT_FOUND_404, description = "User를 찾을 수 없음",
  ...
  @ApiResponses(value = {
      @ApiResponse(
          responseCode = SUCCESS_200, description = "User 목록 조회 성공",
  ...
  @ApiResponses(value = {
      @ApiResponse(
          responseCode = SUCCESS_200, description = "User 온라인 상태가 성공적으로 업데이트됨",
          content = @Content(schema = @Schema(implementation = UserStatus.class))
      ),
      @ApiResponse(
          responseCode = NOT_FOUND_404, description = "해당 User의 UserStatus를 찾을 수 없음",
