...
public interface ChannelApi {
  ...
  @ApiResponses(value = {
      @ApiResponse(
          responseCode = CREATED_201, description = "채널이 성공적으로 생성됨",
          content = @Content(schema = @Schema(implementation = Channel.class))
      ),
      @ApiResponse(
          responseCode = BAD_REQUEST_400, description = "같은 채널이 이미 존재함",
  ...
  @ApiResponses(value = {
      @ApiResponse(
          responseCode = SUCCESS_200, description = "Channel 정보가 성공적으로 수정됨",
          content = @Content(schema = @Schema(implementation = Channel.class))
      ),
      @ApiResponse(
          responseCode = NOT_FOUND_404, description = "Channel을 찾을 수 없음",
          content = @Content(examples = @ExampleObject("Channel with id {channelId} not found"))
      ),
      @ApiResponse(
          responseCode = BAD_REQUEST_400, description = "같은 name를 사용하는 Channel을 이미 존재함",
  ...
  @ApiResponses(value = {
      @ApiResponse(
          responseCode = NO_CONTENT_204, description = "Channel이 성공적으로 삭제됨"
      ),
      @ApiResponse(
          responseCode = NOT_FOUND_404, description = "Channel을 찾을 수 없음",
  ...
  @ApiResponses(value = {
      @ApiResponse(
          responseCode = SUCCESS_200, description = "사용자가 참여중인 채널 목록 조회 성공",
