...
class MessageControllerTest {
  ...
  void createMessage_success() throws Exception {
    ...
    MockMultipartFile mockMultipartFile = new MockMultipartFile("messageCreateRequest", "",
        MediaType.APPLICATION_JSON_VALUE, requestJson.getBytes(
        StandardCharsets.UTF_8));

    MessageDto responseDto = new MessageDto(UUID.randomUUID(), Instant.now(), Instant.now(), "하이",
  ...
  void findByChannelId_success() throws Exception {
    ...
    MessageDto messageDto = new MessageDto(UUID.randomUUID(), Instant.now(), Instant.now(), "안녕하세요",
        channelId, user, null);
